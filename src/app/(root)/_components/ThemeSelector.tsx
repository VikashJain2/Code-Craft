...
import { Atom, CircleOff, Cloud, Github, Laptop, Lightbulb, Moon, Palette, Snowflake, Sun, MoonStar, Ghost,} from "lucide-react";
const THEMES_ICONS: Record<string, React.ReactNode> = {
  ...
  "one-dark-pro": <Laptop className="size-4" />, // Icon for One Dark Pro
  "atom-one-dark": <Atom className="size-4" />, // Icon for Atom One Dark
  dracula: <Ghost className="size-4" />, // Icon for Dracula
  nord: <Snowflake className="size-4" />, // Icon for Nord
  palenight: <MoonStar className="size-4" />, // Icon for Palenight
  "one-light": <Lightbulb className="size-4" />, // Icon for One Light
};
          ...
          >
            ...
            {THEMES.map((t, index) => (
              <motion.button
                ...
                onClick={() => {setTheme(t.id); setIsOpen(!isOpen)}}
              >
                ...
                <div
                  className={`flex items-center justify-center size-8 rounded-lg ${theme === t.id ? "bg-blue-500/10 text-blue-400" : "bg-gray-800/50 text-gray-400"}`}
                >
                  {THEMES_ICONS[t.id] || <CircleOff className="w-4 h-4"/>}
                </div>
                ...
                {theme ===  t.id && (
                    <motion.div className="absolute inset-0 border-2 border-blue-500/30 rounded-lg" transition={{type:"spring",bounce:0.2, duration:0.6}}/>
                )}
              </motion.button>
            ))}
          </motion.div>
