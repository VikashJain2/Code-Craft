...
const Page = () => {
  ...
  const [selectedLanguage, setSeletedLanguage] = useState<string | null>(null);
  ...
  const popularLanguages = languages.splice(0, 5);

  const filteredSnippets = snippets.filter((snippets) => {
    const matchesSearch =
      snippets.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      snippets.language.toLowerCase().includes(searchQuery.toLowerCase()) ||
      snippets.username.toLowerCase().includes(searchQuery.toLowerCase());

    const matchesLanguage =
      !selectedLanguage || snippets.language === selectedLanguage;
...
<div className="relative max-w-5xl mx-auto mb-12 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-5">
          ...
          {popularLanguages.map((lang) => (
            <button
              key={lang}
              onClick={() =>
                setSeletedLanguage(lang === selectedLanguage ? null : lang)
              }
          ...
          {selectedLanguage && (
            <button
              onClick={() => setSeletedLanguage(null)}
        ...
        <motion.div className={`grid gap-6 ${view === "grid" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3": "grid-cols-1 max-w-3xl mx-auto"}`} layout>
